...
from contracthub.interfaces.streamlit.editor.analysis import (
...
from contracthub.interfaces.streamlit.editor.raw_yaml import (
    apply_raw_yaml,
    validate_raw_yaml,
)
...
from contracthub.interfaces.streamlit.editor.styles import (
    catalog_badges_html,
    inject_app_styles,
    section_title,
)
...
def _render_catalog_header() -> None:
    ...
    with actions_col:
        ...
        st.button(
            ...
            width="stretch",
            disabled=True,
            help="Contracts are generated by the data engineering workflow, not created from the UI.",
        )
    ...
    with filter_cols[0]:
        st.text_input(
            "Search",
            key="catalog_search",
            placeholder="Search contracts by data product",
        )
    with filter_cols[1]:
        st.selectbox(
            "Domain",
            options=["All", *_catalog_filter_options("domain")],
            key="catalog_filter_domain",
        )
    with filter_cols[2]:
        st.selectbox(
            "Status",
            options=["All", *_catalog_filter_options("status")],
...
def _render_detail_header(contract: dict[str, Any]) -> None:
    ...
    with title_col:
        st.title("ContractHub")
        st.subheader(
            f"{contract_data_product(contract) or 'Untitled Contract'}  ·  {contract_version(contract) or 'Not defined'}"
        )
    with actions_col:
...
def _render_catalog_view() -> None:
    ...
    section_title(
        "Contract Catalog",
        "Browse contracts, filter business context, and jump into detail editing when needed.",
    )
...
def _render_catalog_card(entry: dict[str, Any]) -> None:
    ...
    with st.container(border=True):
        ...
        st.markdown(
            catalog_badges_html(
                status, entry.get("flags", []), contract_tags(contract)
            ),
            unsafe_allow_html=True,
        )
        st.markdown(
        ...
        st.caption(
            _summary_text(
                ...
                fallback="No usage summary defined.",
            )
        )
        ...
        with action_cols[0]:
            if st.button(
                "View Details", key=f"catalog_view_{entry['id']}", width="stretch"
            ):
...
def _render_detail_view() -> None:
    ...
    if st.session_state.get("editor_analysis_visible") and isinstance(
        st.session_state.get("editor_analysis_result"), dict
    ):
        st.divider()
        # Analyze Results
        section_title(
            "Analyze Results",
            "Review governance impact before saving contract changes.",
        )
...
def _build_demo_catalog_entries() -> list[dict[str, Any]]:
    ...
    legacy_contract.setdefault("description", {})["purpose"] = (
        "Legacy seller payment metrics for decommissioning workflows."
    )
    legacy_contract["description"]["usage"] = (
        "Reference only for backfill and historic validation."
    )
    ...
    breaking_contract.setdefault("description", {})["purpose"] = (
        "Upcoming seller payment contract with pending schema changes."
    )
    breaking_contract["description"]["usage"] = (
        "Used for rollout planning and release readiness checks."
    )
    ...
    draft_contract.setdefault("description", {})["purpose"] = (
        "Draft contract for sandbox settlement testing."
    )
...
def _open_contract(entry: dict[str, Any], *, edit_mode: bool) -> None:
    ...
    if edit_mode and working_contract.get("version") != main_contract.get("version"):
        st.session_state["editor_warning"] = (
            "⚠️ The technical baseline has been updated since this draft was created. Your draft metadata will be preserved, but technical schema fields have been merged. Please review changes before saving."
        )
...
def _handle_analyze() -> None:
    ...
    st.session_state["editor_analysis_visible"] = isinstance(
        st.session_state.get("editor_analysis_result"), dict
    )
