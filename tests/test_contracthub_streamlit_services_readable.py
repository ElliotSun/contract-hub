...
def test_contract_service_lists_contract_metadata_and_editability(
    ...
    service = ContractService(
        contracts_dir=contracts_dir, drafts_dir=tmp_path / ".contracthub" / "drafts"
    )
...
def test_contract_service_list_contracts_uses_environment_configured_contract_root(
    sample_odcs_dict, tmp_path, monkeypatch
):
    ...
    contracts = contract_service_module.list_contracts(
        user={"tenant": "tenant-a", "role": "editor"}
    )
...
def test_contract_service_list_contracts_supports_adls2_contract_root(monkeypatch):
    ...
    service = ContractService(
        contracts_dir="abfss://contracts@acct.dfs.core.windows.net/contracts"
    )
    contracts = service.list_contracts(user={"tenant": "tenant-a", "role": "editor"})
...
def test_contract_service_get_draft_returns_main_contract_when_no_draft_exists(
    sample_odcs_dict, tmp_path
):
    contracts_dir = tmp_path / "contracts"
    ...
    draft = service.get_draft(
        "seller-payments", user={"tenant": "tenant-a", "role": "editor", "id": "alice"}
    )

    assert draft["id"] == "seller-payments"
    assert draft["tenant"] == "tenant-a"
    assert not (
        tmp_path / ".contracthub" / "drafts" / "alice" / "seller-payments.yaml"
    ).exists()


def test_contract_service_get_draft_prefers_saved_user_draft(
    sample_odcs_dict, tmp_path
):
    contracts_dir = tmp_path / "contracts"
...
def test_contract_service_save_draft_persists_draft_without_overwriting_main(
    sample_odcs_dict, tmp_path
):
    contracts_dir = tmp_path / "contracts"
    drafts_dir = tmp_path / ".contracthub" / "drafts"
    ...
    saved = service.save_draft(
        draft, user={"tenant": "tenant-a", "role": "editor", "id": "alice"}
    )
...
def test_contract_service_save_draft_preserves_read_only_contract_fields_from_main(
    sample_odcs_dict, tmp_path
):
    contracts_dir = tmp_path / "contracts"
    drafts_dir = tmp_path / ".contracthub" / "drafts"
...
def test_contract_service_save_draft_preserves_schema_and_property_technical_fields(
    sample_odcs_dict, tmp_path
):
    contracts_dir = tmp_path / "contracts"
    drafts_dir = tmp_path / ".contracthub" / "drafts"
    ...
    main_contract["schema"][0]["quality"] = [
    ...
    main_contract["schema"][0]["properties"][0]["description"] = (
    ...
    draft["schema"][0]["quality"] = [
        {"name": "schema_rule", "metric": "rowCount", "mustBeGreaterThan": 1}
    ]
    ...
    draft["schema"][0]["properties"][0]["quality"] = [
        {"name": "field_rule", "metric": "nullValues", "mustBe": 0}
    ]
    ...
    assert saved_field["quality"] == [
        {"name": "field_rule", "metric": "nullValues", "mustBe": 0}
    ]


def test_contract_service_save_draft_rejects_unauthorized_user(
    sample_odcs_dict, tmp_path
):
    contracts_dir = tmp_path / "contracts"
    main_contract = dict(sample_odcs_dict)
    ...
    try:
        service.save_draft(
            main_contract, user={"tenant": "tenant-b", "role": "editor", "id": "bob"}
        )
...
def test_contract_service_rejects_non_admin_edit_when_contract_tenant_is_missing(
    sample_odcs_dict, tmp_path
):
    contracts_dir = tmp_path / "contracts"
    main_contract = dict(sample_odcs_dict)
    ...
    with pytest.raises(PermissionError, match="seller-payments"):
        service.get_draft(
            "seller-payments",
            user={"tenant": "tenant-a", "role": "editor", "id": "alice"},
        )


def test_contract_service_allows_admin_edit_when_contract_tenant_is_missing(
    sample_odcs_dict, tmp_path
):
    contracts_dir = tmp_path / "contracts"
    main_contract = dict(sample_odcs_dict)
    ...
    draft = service.get_draft(
        "seller-payments", user={"tenant": "", "role": "admin", "id": "alice"}
    )
    assert draft["id"] == "seller-payments"
...
def test_contract_service_save_draft_preserves_technical_fields_after_schema_and_property_reordering(
    ...
    draft["schema"][1]["properties"] = [
        draft["schema"][1]["properties"][1],
        draft["schema"][1]["properties"][0],
    ]
...
def test_contract_service_analyze_draft_delegates_main_vs_draft_to_governance(
    sample_odcs_dict, tmp_path, monkeypatch
):
    contracts_dir = tmp_path / "contracts"
    main_contract = dict(sample_odcs_dict)
    ...
    monkeypatch.setattr(
        contract_service_module.governance_service, "analyze_contracts", fake_analyze
    )
    ...
    result = service.analyze_draft(
        draft_contract, user={"tenant": "tenant-a", "role": "editor", "id": "alice"}
    )
...
def test_contract_service_analyze_draft_convenience_wrapper(
    sample_odcs_dict, tmp_path, monkeypatch
):
    contracts_dir = tmp_path / "contracts"
    main_contract = dict(sample_odcs_dict)
    ...
    monkeypatch.setenv("CONTRACTHUB_CONTRACTS_DIR", str(contracts_dir))
    monkeypatch.setenv(
        "CONTRACTHUB_DRAFTS_DIR", str(tmp_path / ".contracthub" / "drafts")
    )
    monkeypatch.setattr(
    ...
    result = analyze_draft(
        main_contract, user={"tenant": "tenant-a", "role": "editor", "id": "alice"}
    )
...
def test_governance_service_analyze_delegates_to_merge_engine(
    sample_odcs_dict, sample_odcs_model, monkeypatch
):
    source_yaml = yaml.safe_dump(sample_odcs_dict, sort_keys=False)
...
def test_governance_service_apply_delegates_to_merge_engine(
    sample_odcs_dict, sample_odcs_model, monkeypatch
):
    source_yaml = yaml.safe_dump(sample_odcs_dict, sort_keys=False)
    target_yaml = yaml.safe_dump(sample_odcs_dict, sort_keys=False)
